...
async function archiveDocument(documentId,button){
  try{
    ...
    archive.invalidate();
    await loadDashboard();
    if(!$("#archivePanel")?.classList.contains("hidden"))await archive.loadWorkspace(true);
...
function bindEvents(){
  ...
  $("#refreshArchive")?.addEventListener("click",async()=>{
    ...
    try{setBusy(btn,true,"↻");await archive.loadWorkspace(true);}finally{setBusy(btn,false);}
  });
  ...
  $("#closeArchiveTrace")?.addEventListener("click",()=>archive.renderTrace(null));
  $("#openArchivedDocument")?.addEventListener("click",()=>{
    const id=archive.activeDocumentId();
    if(id)archive.openDocument(id);
  });
  $("#printArchiveTrace")?.addEventListener("click",archive.printTraceability);
  ...
  $("#mySignatureList")?.addEventListener("click",e=>{
    ...
    if(open)archive.openDocument(open.dataset.openCloudDoc);
  });
  $("#sentSignatureList")?.addEventListener("click",e=>{
    const open=e.target.closest("[data-open-cloud-doc]");if(open){archive.openDocument(open.dataset.openCloudDoc);return;}
